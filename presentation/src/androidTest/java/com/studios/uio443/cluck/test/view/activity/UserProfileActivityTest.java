 ...
 */
package com.studios.uio443.cluck.test.view.activity;
...
public class UserProfileActivityTest extends ActivityInstrumentationTestCase2<UserProfileActivity> {
  ...
  public void testContainsUserDetailsFragment() {
    Fragment userDetailsFragment =
            userProfileActivity.getFragmentManager().findFragmentById(R.id.fragmentContainer);
  ...
  public void testLoadUserHappyCaseViews() {
    ...
    onView(withId(R.id.tv_votes)).check(matches(isDisplayed()));
  }

  public void testLoadUserHappyCaseData() {
    onView(withId(R.id.tv_fullname)).check(matches(withText("John Sanchez")));
    onView(withId(R.id.tv_points)).check(matches(withText("dmedina@katz.edu")));
    onView(withId(R.id.tv_votes)).check(matches(withText("4523")));
  }

  private Intent createTargetIntent() {
    return UserProfileActivity.getCallingIntent(getInstrumentation().getTargetContext(), FAKE_USER_ID);
  }
}
